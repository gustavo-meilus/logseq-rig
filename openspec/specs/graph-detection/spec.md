# graph-detection Specification

## Purpose

Provides one read-only, machine-readable description of a supported Logseq OG graph so every downstream command uses the same discovered conventions.
...
#### Scenario: Non-graph folder is inspected
...
#### Scenario: Graph uses custom directories
...
#### Scenario: Graph declares a custom journal format

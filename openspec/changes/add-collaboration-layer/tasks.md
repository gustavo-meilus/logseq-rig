...
- [ ] 2.1 Add `.github/CODEOWNERS` assigning `@gustavo-meilus` to all files and `.github/`, then verify GitHub recognizes the file after the pull request is opened.
...
- [ ] 3.1 Add `.github/workflows/ci.yml` for `main` pushes and pull requests using Python 3.11, `pip install .`, unit tests, fast release validation, and whitespace checking, then verify the workflow appears in GitHub Actions.
...
- [ ] 3.3 Confirm the workflow succeeds on `main`, then record its exact required-status-check name for the branch ruleset.
...
- [ ] 4.2 Create a `main` branch ruleset that blocks force pushes and deletions and requires pull requests, the successful CI check, and resolved conversations; keep required approvals disabled until a second trusted maintainer exists, then verify the ruleset targets the default branch.

...
The installation SHALL provide one repository-local Vault Rig Skill that contains reusable retrieval and safe-edit procedures and delegates deterministic mechanics to Vault Rig commands.

#### Scenario: Logseq-aware procedure is needed
- **WHEN** a task matches the Vault Rig Skill
...
Independent read-only review MAY be documented for high-risk semantic restructuring but SHALL NOT be required for routine Vault Rig use.

...
Vault Rig needs a repository foundation that separates its reusable managed payload from development assets and from every target vault's canonical Logseq Markdown. Establishing those ownership boundaries first prevents later lifecycle and tooling work from coupling to one personal vault.
...
### New Capabilities
- `repository-foundation`: Defines Vault Rig repository boundaries, target-vault ownership boundaries, and the stable bootstrap entrypoint.

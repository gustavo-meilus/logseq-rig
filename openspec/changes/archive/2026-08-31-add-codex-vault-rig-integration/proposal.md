...
Generic Codex behavior can flatten Logseq block structure, infer unsafe paths, and omit graph verification. A small repository-local operating contract can route work through deterministic Logseq Vault Rig commands without loading a Logseq manual into every session.
...
### New Capabilities
- `codex-integration`: Provides minimum repository-local Codex instructions, Skill routing, lifecycle hooks, and authority defaults for safe Logseq Vault Rig use.

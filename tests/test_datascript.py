...
from vault_rig.datascript import Client, QueryError, execute, probe
from vault_rig.retrieval import load
from vault_rig.detection import detect


def vault(root: Path) -> None:
...
class DataScriptTests(unittest.TestCase):
    def setUp(self):
        self.environment = {"VAULT_RIG_LOGSEQ_ENDPOINT": "http://127.0.0.1:1234/query", "VAULT_RIG_LOGSEQ_TOKEN": "secret-token"}

    def pages(self):
        temporary = tempfile.TemporaryDirectory(); self.addCleanup(temporary.cleanup)
        root = Path(temporary.name); vault(root)
    ...
    def test_configuration_and_registry_fail_before_http(self):
        ...
        with patch.dict(os.environ, {**self.environment, "VAULT_RIG_LOGSEQ_ENDPOINT": "https://example.test"}, clear=True):
            with self.assertRaisesRegex(QueryError, "loopback"): Client.from_environment()
        with patch("vault_rig.datascript.build_opener") as opener, patch.dict(os.environ, self.environment, clear=True):
    ...
    def test_success_probe_normalization_and_unresolved_evidence(self):
        with patch.dict(os.environ, self.environment, clear=True), patch("vault_rig.datascript.build_opener") as opener:
